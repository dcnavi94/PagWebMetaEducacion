        ...
        function switchMoodlePanel(panel) {
            ...
            if (panel === 'courses') {
                loadMoodleCourseCategories();
                loadMoodleCoursesList();
            } else if (panel === 'groups') {
                loadMoodleAssignmentOptions();
            } else if (panel === 'accounts') {
                loadMoodleAccounts();
            } else if (panel === 'tokens') {
                loadMoodleConfiguration();
            }
        }

        async function loadMoodleAssignmentOptions() {
            const courseSelect = document.getElementById('moodleAssignmentCourse');
            const studentSelect = document.getElementById('moodleAssignmentStudent');
            const teacherSelect = document.getElementById('moodleAssignmentTeacher');
            const groupSelect = document.getElementById('moodleAssignmentLocalGroup');
            ...
            if (courseSelect) {
                courseSelect.innerHTML = courses.length
                    ? '<option value="">Seleccionar curso Moodle...</option>' + courses.map(course =>
                        `<option value="${Number(course.id)}">${course.displayname || course.fullname || course.shortname || `Curso ${course.id}`}</option>`
                    ).join('')
                    : '<option value="">No hay cursos Moodle disponibles</option>';
            }

            if (studentSelect) {
                const students = [...(allStudents || [])].sort((a, b) =>
                    String(a.full_name || a.username || '').localeCompare(String(b.full_name || b.username || ''), 'es')
                );
                studentSelect.innerHTML = students.length
                    ? '<option value="">Seleccionar alumno...</option>' + students.map(student =>
                        `<option value="${student.username}">${student.full_name || student.username} · ${student.username}</option>`
                    ).join('')
                    : '<option value="">No hay alumnos locales disponibles</option>';
            }

            if (teacherSelect) {
                const teachers = [...(allTeachers || [])].sort((a, b) =>
                    String(a.full_name || a.username || '').localeCompare(String(b.full_name || b.username || ''), 'es')
                );
                teacherSelect.innerHTML = teachers.length
                    ? '<option value="">Seleccionar docente...</option>' + teachers.map(teacher =>
                        `<option value="${teacher.username}">${teacher.full_name || teacher.username} · ${teacher.username}</option>`
                    ).join('')
                    : '<option value="">No hay docentes locales disponibles</option>';
            }

            if (groupSelect) groupSelect.innerHTML = '<option value="">Cargando grupos administrativos...</option>';
            try {
                const response = await fetch('/admin/groups', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (!response.ok) {
                    if (groupSelect) groupSelect.innerHTML = '<option value="">No se pudieron cargar los grupos</option>';
                } else {
                    const groups = await response.json();
                    if (groupSelect) {
                        groupSelect.innerHTML = groups.length
                            ? '<option value="">Seleccionar grupo administrativo...</option>' + groups.map(group =>
                                `<option value="${Number(group.group_id)}" data-total="${Number(group.total || 0)}">${group.grupo} · ${Number(group.total || 0)} alumno(s)</option>`
                            ).join('')
                            : '<option value="">No hay grupos administrativos activos</option>';
                        groupSelect.onchange = updateMoodleAssignmentGroupInfo;
                    }
                }
            } catch (error) {
                console.error(error);
                if (groupSelect) groupSelect.innerHTML = '<option value="">Error cargando grupos</option>';
            }

            updateMoodleAssignmentGroupInfo();
            await loadMoodleGroupsList();
        }

        function updateMoodleAssignmentGroupInfo() {
            const select = document.getElementById('moodleAssignmentLocalGroup');
            const info = document.getElementById('moodleAssignmentGroupInfo');
            if (!select || !info) return;
            const selected = select.options[select.selectedIndex];
            if (!selected?.value) {
                info.textContent = 'Se creará o reutilizará en Moodle un grupo con el mismo nombre.';
                return;
            }
            info.textContent = `${selected.dataset.total || 0} alumno(s) del ciclo activo serán enrolados y agregados al grupo "${selected.textContent.split(' · ')[0]}".`;
        }

        async function assignStudentToMoodleCourse() {
            const courseId = Number.parseInt(document.getElementById('moodleAssignmentCourse')?.value || '', 10);
            const username = document.getElementById('moodleAssignmentStudent')?.value || '';
            if (!courseId || !username) {
                showToast('Selecciona un curso Moodle y un alumno', true);
                return;
            }
            try {
                const response = await fetch('/admin/moodle/enrol-student', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                ...
                if (!response.ok) {
                    showToast(await extractApiErrorMessage(response, 'No se pudo inscribir al alumno'), true);
                    return;
                }
                const result = await response.json();
                showToast(result.message || 'Alumno inscrito correctamente');
            } catch (error) {
                console.error(error);
                showToast('Error de conexión al inscribir al alumno', true);
            }
        }

        async function assignTeacherToMoodleCourse() {
            const courseId = Number.parseInt(document.getElementById('moodleAssignmentCourse')?.value || '', 10);
            const username = document.getElementById('moodleAssignmentTeacher')?.value || '';
            if (!courseId || !username) {
                showToast('Selecciona un curso Moodle y un docente', true);
                return;
            }
            try {
                const response = await fetch('/admin/moodle/enrol-teacher', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                ...
                if (!response.ok) {
                    showToast(await extractApiErrorMessage(response, 'No se pudo asignar al docente'), true);
                    return;
                }
                const result = await response.json();
                showToast(result.message || 'Profesor asignado correctamente');
            } catch (error) {
                console.error(error);
                showToast('Error de conexión al asignar al docente', true);
            }
        }

        async function assignLocalGroupToMoodleCourse() {
            const courseId = Number.parseInt(document.getElementById('moodleAssignmentCourse')?.value || '', 10);
            const groupId = Number.parseInt(document.getElementById('moodleAssignmentLocalGroup')?.value || '', 10);
            if (!courseId || !groupId) {
                showToast('Selecciona un curso Moodle y un grupo administrativo', true);
                return;
            }
            if (!confirm('Se enrolarán en Moodle todos los alumnos activos de este grupo. ¿Continuar?')) return;

            try {
                const response = await fetch(`/admin/moodle/local-groups/${groupId}/enrol`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                ...
                if (!response.ok) {
                    showToast(await extractApiErrorMessage(response, 'No se pudo sincronizar el grupo'), true);
                    return;
                }
                const result = await response.json();
                const failed = (result.failures || []).length;
                showToast(
                    `${result.group_name}: ${result.students_enrolled} alumno(s) inscritos` +
                    (failed ? `, ${failed} con error.` : '.')
                );
                await loadMoodleGroupsList();
            } catch (error) {
                console.error(error);
                showToast('Error de conexión al sincronizar el grupo', true);
            }
        }

        async function loadMoodleCourseCategories() {
            const select = document.getElementById('moodleCourseCategory');
            if (!select) return;
            try {
                const response = await fetch('/admin/moodle/categories', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                select.innerHTML = categories.map(category =>
                    `<option value="${category.id}">${category.name || `Categoría ${category.id}`}</option>`
                ).join('');
            } catch (error) {
                console.error(error);
            }
                        ...
                        ? adminNotifications.map(item => {
                            ...
                            return `
                                <div class="px-2 py-2 border-bottom">
                                    <div class="fw-semibold ${levelClass}">${escapeHtml(item.title || 'Notificacion')}${statusBadge}</div>
        ...
        async function goToMoodleSite() {
            const popup = window.open('', '_blank');
            try {
                const response = await fetch('/admin/moodle/launch', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (!response.ok) {
                    if (popup) popup.close();
                    showToast(await extractApiErrorMessage(response, 'No se pudo iniciar sesión como administrador de Moodle'), true);
                    return;
                }
                const payload = await response.json();
                if (popup) {
                    popup.location.href = payload.sso_url;
                } else {
                    window.location.href = payload.sso_url;
                }
            } catch (error) {
                console.error(error);
                if (popup) popup.close();
                showToast('Error de conexión al abrir Moodle como administrador', true);
            }
        }
        ...
        function renderMoodleRequiredPermissions(items = []) {
            const container = document.getElementById('moodleRequiredPermissions');
            if (!container) return;
            if (!items.length) {
                container.innerHTML = '<div class="text-muted py-3">No se recibió la lista de permisos.</div>';
                return;
            }
            container.innerHTML = items.map(item => `
                <div class="moodle-permission-row">
                    <code class="small">${item.name || '-'}</code>
                    <span class="small text-muted">${item.description || 'Función requerida por la integración.'}</span>
                    <span class="badge rounded-pill ${item.enabled ? 'bg-success-subtle text-success border border-success-subtle' : 'bg-danger-subtle text-danger border border-danger-subtle'}">
                        <i class="bi ${item.enabled ? 'bi-check-circle-fill' : 'bi-x-circle-fill'} me-1"></i>${item.enabled ? 'Habilitado' : 'Faltante'}
                    </span>
                </div>
            `).join('');
        }

        async function loadMoodleConfiguration(showNotification = false) {
            const statusText = document.getElementById('moodleConnectionStatusText');
            const statusDot = document.getElementById('moodleConfigStatusDot');
            try {
                const response = await fetch('/admin/moodle/configuration', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (!response.ok) {
                    throw new Error(await extractApiErrorMessage(response, 'No se pudo consultar la configuración Moodle'));
                }
                const config = await response.json();
                if (statusText) statusText.textContent = config.connected ? 'Moodle conectado' : 'Moodle sin conexión';
                if (statusDot) statusDot.className = `moodle-config-dot ${config.connected ? 'connected' : 'error'}`;

                const setText = (id, value) => {
                    const element = document.getElementById(id);
                    if (element) element.textContent = value;
                };
                setText('moodleConfigToken', config.token_display || 'No configurado');
                setText('moodleConfigApi', config.connected ? 'Conectada' : 'Sin respuesta');
                setText('moodleConfigPermissions', `${config.enabled_required_count || 0} / ${config.required_count || 0}`);
                setText('moodleConfigAutoLogin', config.auto_login_enabled ? 'Activado' : 'Desactivado');
                setText('moodleConfigBaseUrl', config.base_url || 'No configurado');
                setText('moodleConfigPublicUrl', config.public_url || 'No configurado');
                setText('moodleConfigStudentRole', `ID ${config.student_role_id ?? '-'} · contexto ${config.role_context_level || '-'}`);
                setText('moodleConfigTeacherRole', `ID ${config.teacher_role_id ?? '-'} · contexto ${config.role_context_level || '-'}`);

                const missingCount = (config.missing_functions || []).length;
                const badge = document.getElementById('moodleMissingPermissionsBadge');
                if (badge) {
                    badge.textContent = missingCount ? `${missingCount} permisos faltantes` : 'Todos los permisos listos';
                    badge.className = `badge rounded-pill ${missingCount ? 'bg-danger-subtle text-danger border border-danger-subtle' : 'bg-success-subtle text-success border border-success-subtle'}`;
                }
                renderMoodleRequiredPermissions(config.required_functions || []);

                const info = document.getElementById('moodleConnectionInfo');
                if (info) {
                    info.innerHTML = `
                        <div class="row g-2">
                            <div class="col-md-6"><strong>Sitio:</strong> ${config.site_name || 'No disponible'}</div>
                            <div class="col-md-6"><strong>Versión:</strong> ${config.moodle_release || 'No disponible'}</div>
                            <div class="col-md-6"><strong>Usuario del token:</strong> ${config.token_username || 'No disponible'}</div>
                            <div class="col-md-6"><strong>Token:</strong> ${config.token_display || 'No configurado'}</div>
                        </div>
                        ${config.last_error ? `<div class="alert alert-danger mt-3 mb-0"><strong>Último error:</strong> ${config.last_error}</div>` : ''}
                    `;
                }
                if (showNotification) showToast(config.connected ? 'Conexión Moodle validada correctamente' : 'Moodle respondió con incidencias', !config.connected);
            } catch (error) {
                console.error(error);
                if (statusText) statusText.textContent = 'Error al validar Moodle';
                if (statusDot) statusDot.className = 'moodle-config-dot error';
                const info = document.getElementById('moodleConnectionInfo');
                if (info) info.innerHTML = `<span class="text-danger">${error.message || 'Error de conexión'}</span>`;
                if (showNotification) showToast(error.message || 'No se pudo validar Moodle', true);
            }
        }

        async function testMoodleConfiguration() {
            await Promise.all([loadMoodleConfiguration(true), loadMoodleHealth(), loadMoodleFunctions()]);
        }
            ...
            try {
                ...
                tbody.innerHTML = moodleFunctions.map((fn, idx) => `
                    <tr>
                        ...
                        <td>${fn.description || inferFunctionDescription(fn.name || '')}</td>
                    </tr>
                `).join('');
            } catch (error) {
                console.error(error);
        ...
        function selectMoodleAccount(username) {
            ...
            document.getElementById('moodleCredPassword').value = selected.moodle_password || 'unives12345';
        ...
        async function saveMoodleCredential() {
            ...
            if (moodlePassword && moodlePassword.length < 8) {
            ...
            try {
                ...
                const result = await response.json();
                showToast(result.password_generated
                    ? `Contraseña aleatoria generada: ${result.moodle_password}`
                    : 'Credenciales Moodle guardadas correctamente');
        ...
        async function createMoodleCourse() {
            ...
            try {
                ...
                showToast(result.category_adjusted
                    ? `Curso creado en Moodle (ID ${course.id || 'nuevo'}) en la categoría disponible`
                    : `Curso creado en Moodle (ID ${course.id || 'nuevo'})`);
                await loadMoodleCoursesList();
            } catch (error) {
                console.error(error);
