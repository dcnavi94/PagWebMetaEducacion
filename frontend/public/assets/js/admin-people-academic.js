                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-switch d-inline-block me-2">
                                        <input class="form-check-input" type="checkbox" id="conciliate_${p.id}" ${p.is_conciliated ? 'checked' : ''} onchange="togglePaymentConciliation(${p.id}, this.checked)">
                                        <label class="form-check-label small" for="conciliate_${p.id}">Conciliado</label>
                                    </div>
                                </td>
                                <td>
                                    ${p.receipt_url ? `<a href="${p.receipt_url}" target="_blank" class="btn btn-sm btn-outline-info rounded-pill me-1" title="Ver Comprobante"><i class="bi bi-file-earmark-text"></i></a>` : ''}
                                    <button class="btn btn-sm btn-outline-secondary rounded-pill me-1" title="Subir Comprobante" onclick="document.getElementById('uploadReceiptPaymentId').value=${p.id}; new bootstrap.Modal(document.getElementById('uploadReceiptModal')).show();"><i class="bi bi-upload"></i></button>
        ...
        async function togglePaymentConciliation(paymentId, isConciliated) {
            try {
                const response = await fetch(`/admin/payments/${paymentId}`, {
                    method: 'PUT',
                    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify({ is_conciliated: isConciliated })
                });
                if (response.ok) {
                    showToast('Estado de conciliacion actualizado', 'success');
                } else {
                    const error = await response.json();
                    showToast(error.detail || 'Error al actualizar', 'danger');
                    // Revert UI toggle
                    const el = document.getElementById(`conciliate_${paymentId}`);
                    if (el) el.checked = !isConciliated;
                }
            } catch (error) {
                showToast('Error de conexion', 'danger');
                const el = document.getElementById(`conciliate_${paymentId}`);
        ...
        async function uploadPaymentReceipt(event) {
            event.preventDefault();
            const paymentId = document.getElementById('uploadReceiptPaymentId').value;
            const fileInput = document.getElementById('uploadReceiptFile');
            if (!fileInput.files.length) {
                showToast('Selecciona un archivo', 'danger');
                return;
            }
            ...
            try {
                const response = await fetch(`/admin/payments/${paymentId}/receipt`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` },
                    body: formData
                });
                if (response.ok) {
                    ...
                    const error = await response.json();
                    showToast(error.detail || 'Error al subir comprobante', 'danger');
                }
            } catch (error) {
                showToast('Error de conexion', 'danger');
            }
